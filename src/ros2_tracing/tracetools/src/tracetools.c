...
void TRACEPOINT(
  ...
  const int64_t received_timestamp)
{
  CONDITIONAL_TP(
    rclcpp_take,
    message,
...
void TRACEPOINT(
  rclpy_publish,
  const uint64_t publisher_handle,
  const char * node_name,
  ...
  const int64_t received_timestamp)
{
  CONDITIONAL_TP(
    rclpy_take,
    node_name, message, topic_name, source_timestamp, received_timestamp);
}
