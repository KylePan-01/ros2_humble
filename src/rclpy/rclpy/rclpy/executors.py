...
async def await_or_execute(callback: Union[Callable, Coroutine], *args, node_name="") -> Any:
    """Await a callback if it is a coroutine, else execute it."""
    _rclpy.rclpy_trace_callback_start(id(callback), node_name)
    ...
    _rclpy.rclpy_trace_callback_end(id(callback), node_name)
    return response
...
class Executor:
    ...
    async def _execute_timer(self, tmr, _, node_name="timer"):
        await await_or_execute(tmr.callback, node_name=node_name)
    ...
    def _take_subscription(self, sub):
        with sub.handle:
            ...
            if msg_info is not None:
                msg = msg_info[0]
                _rclpy.rclpy_trace_take(sub.get_node_name, id(msg),
                                sub.topic_name, msg_info[1]["source_timestamp"], msg_info[1]["received_timestamp"])
                return msg_info[0]
        return None

    async def _execute_subscription(self, sub, msg, node_name="subscription"):
        if msg:
            await await_or_execute(sub.callback, msg, node_name=node_name)
            ...
            with work_tracker:
                ...
                try:
                    if not flag:
                        ...
                        await call_coroutine(entity, arg, node_name=node.get_name())
                finally:
                    ...
                    if tmr.handle.pointer in timers_ready:
                        ...
                        if tmr.handle.is_timer_ready():
                            if tmr.callback_group.can_execute(tmr):
                                handler = self._make_handler(
                                    tmr, node, self._take_timer, self._execute_timer, flag=True)
                                yielded_work = True
                ...
                for sub in node.subscriptions:
                    if sub.handle.pointer in subs_ready:
                        if sub.callback_group.can_execute(sub):
                            handler = self._make_handler(
                                sub, node, self._take_subscription, self._execute_subscription, flag=True)
                            yielded_work = True
