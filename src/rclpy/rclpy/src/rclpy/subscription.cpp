...
std::string
Subscription::get_node_name()
{
  const char * node_name = node_.get_node_name();
  if (!node_name) {
    throw RCLError("failed to get node name");
  }
...
define_subscription(py::object module)
{
  ...
  .def(
    "get_node_name", &Subscription::get_node_name,
    "Get the name of the node associated with the subscription.");
}
}  // namespace rclpy
